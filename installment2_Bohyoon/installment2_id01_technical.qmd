...
```{r remove_outliers}
fullmodel <- lm(PRSM ~ FICO + TotalAmtOwed + Stress + 
                  WomanOwned + CorpStructure + NAICS + I(Months^(-1)) + 
                  FICO:WomanOwned, data = installment2_id01)
plot(fullmodel,1)
...
fullmodel <- lm(PRSM ~ FICO + TotalAmtOwed + Stress + 
                  WomanOwned + CorpStructure + NAICS + I(Months^(-1)) + 
                  FICO:WomanOwned, data = installment2_id01_clean)
...
fullmodel <- lm(PRSM ~ ., data = installment2_id01_clean[,c("PRSM", "FICO", "TotalAmtOwed","Volume","Stress",
                         "p_delinquent","WomanOwned", "CorpStructure" , "NAICS" , "Months")])
fullmodel <- lm(PRSM ~ FICO + TotalAmtOwed + Stress + 
                  WomanOwned + CorpStructure + NAICS + I(Months^(-1)) + 
                  FICO:WomanOwned, data = installment2_id01_clean)
summary(fullmodel)
...
#cv_lasso <- cv.glmnet(x = installment2_id01_clean[,-c(1)], y = installment2_id01_clean[,c(1)], alpha = 1, nfolds = 10)

# TODO: talk about interaction terms?
x <- model.matrix(PRSM ~ .^2, data = installment2_id01_clean[,c("PRSM", "FICO", "TotalAmtOwed","Volume","Stress",
                         "p_delinquent","WomanOwned", "CorpStructure" , "NAICS" , "Months")])[, -1]
...
crPlots(lm(PRSM~NAICS, data = installment2_id01_clean))
