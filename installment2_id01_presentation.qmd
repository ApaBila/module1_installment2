...
-   Result and Interpretation (Main Effects and Interactions)
-   Outlines Avenues for Future Development or Improvement

## Data Preprocessing

-   We load and preprocess the data to ensure the nonnumerical variables aren't considered numerical (FICO, NAICS, WomanOwned).
...
-   Months shows non-linearity; we transform Months and Num_Delinquent to approximately its inverse from the Box-Tidwell test.
...
-   The transformed Month predictor is discernible, while Num_Delinquent is not in the main effect model. Therefore, we remove Num_Delinquent for predictor selection.
...
par(mfrow = c(1, 2))
crPlots(lm(PRSM ~ Months, data = installment2_id01_clean), xlab = "residual vs. fitted for Months")
crPlots(lm(PRSM ~ I(Months^(lambda_months)), data = installment2_id01_clean), xlab = "residual vs. fitted for Months transformed")
```
...
## Result and Interpretation - Main Effects
...
## Result and Interpretation - Interactions

-   The negative effects of lower FICO categories are decreased if the business is women-owned.
-   For the excellent and very good FICO businesess, older businesses perform less than the good FICO businesses.
...
-   The older the corporations are, the more beneficial they are compared to LLCs.

## Outlines Avenues for Future Development or Improvement
-   We may conduct LASSO first rather than stepwise regression to estimate the effect of Num_Delinquent in the future.
